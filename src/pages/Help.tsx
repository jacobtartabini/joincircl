...
const Help = () => {
  ...
  const toggleFAQ = (question: string) => {
    setOpenFAQs(current => current.includes(question) ? current.filter(q => q !== question) : [...current, question]);
  };
        ...
        <CardContent className="space-y-6">
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {faqItems.map(faq => <Collapsible key={faq.question} open={openFAQs.includes(faq.question)} onOpenChange={() => toggleFAQ(faq.question)} className="border rounded-md overflow-hidden">
                  ...
                  <CollapsibleContent className="p-4 pt-3 border-t">
                    <p className="text-sm text-muted-foreground py-0">{faq.answer}</p>
                  </CollapsibleContent>
                </Collapsible>)}
            </div>
          </div>
