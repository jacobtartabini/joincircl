...
export default function Arlo() {
    ...
    return sections.map((section, index) => {
      if (section.includes('**') && section.includes('**')) {
        ...
        return <div key={index} className="mb-4">
            {parts.map((part, partIndex) => partIndex % 2 === 1 ? <strong key={partIndex} className="text-gray-900 dark:text-white font-semibold text-base">{part}</strong> : <span key={partIndex}>{part}</span>)}
          </div>;
      }
      if (section.includes('•') || section.includes('-')) {
        ...
        return <div key={index} className="mb-4">
            ...
            if (line.trim().startsWith('•') || line.trim().startsWith('-')) {
              return <div key={lineIndex} className="flex items-start gap-3 mb-2">
  ...
  if (isMobile) {
    return <div className="h-full flex flex-col bg-gradient-to-br from-gray-50 to-blue-50/20 dark:from-gray-900 dark:to-blue-900/20">
        {/* Mobile Header */}
        ...
        <div className="flex-1 min-h-0">
          <ScrollArea className="h-full">
            <div className="p-6 pb-24">
              {!activeConversation || activeConversation.messages.length <= 1 ? <ConversationStarters onSelectPrompt={handlePromptSelect} /> : <AnimatePresence>
                  {activeConversation.messages.slice(1).map(message => <motion.div key={message.id} initial={{
                opacity: 0,
                y: 20,
                scale: 0.95
              }} animate={{
                ...
                ease: "easeOut"
                      ...
                      <div className={cn("max-w-[85%] rounded-3xl px-5 py-3 relative group", message.role === 'user' ? "bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-lg" : "glass-card text-gray-900 dark:text-white")}>
                        {message.role === 'user' ? <p className="text-base leading-relaxed">{message.content}</p> : <div className="prose prose-sm max-w-none">
                            {formatMessage(message.content)}
                          </div>}
                        
                        <div className={cn("text-xs mt-2 opacity-70", message.role === 'user' ? "text-white/70" : "text-gray-500 dark:text-gray-400")}>
        ...
        <div className="flex-shrink-0 bg-white/90 dark:bg-black/90 backdrop-blur-sm border-t border-white/20 dark:border-white/10 p-6 pb-8">
          <PureMultimodalInput chatId={`arlo-mobile-${activeConversationId}`} messages={activeConversation?.messages || []} attachments={attachments} setAttachments={setAttachments} onSendMessage={handleSendMessage} onStopGenerating={handleStopGenerating} isGenerating={isLoading} canSend={true} selectedVisibilityType="private" value={inputValue} onChange={setInputValue} />
        </div>
      </div>;
  }
  return <div className="h-screen flex bg-gradient-to-br from-gray-50 to-blue-50/30 dark:from-gray-900 dark:to-blue-900/30">
      {/* Sidebar */}
      <ConversationSidebar conversations={conversations} activeConversationId={activeConversationId} onSelectConversation={setActiveConversationId} onCreateConversation={createNewConversation} onDeleteConversation={deleteConversation} onRenameConversation={renameConversation} />
      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        <div className="flex-shrink-0 bg-white/60 dark:bg-black/20 backdrop-blur-sm border-b border-white/20 dark:border-white/10 p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400">Arlo, your intelligent relationship companion</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 relative">
          <ScrollArea className="h-full">
            <div className="max-w-4xl mx-auto">
              {!activeConversation || activeConversation.messages.length <= 1 ? <ConversationStarters onSelectPrompt={handlePromptSelect} /> : <div className="p-6 space-y-6">
                  {activeConversation.messages.slice(1).map(message => <motion.div key={message.id} initial={{
                opacity: 0,
                y: 20
              }} animate={{
                ...
                duration: 0.3
                      ...
                      <div className={cn("max-w-[80%] rounded-2xl p-4 relative", message.role === 'user' ? 'bg-blue-500 text-white' : 'glass-card text-gray-900 dark:text-white')}>
                        {message.role === 'user' ? <p className="leading-relaxed">{message.content}</p> : <div className="prose prose-sm max-w-none">
                            {formatMessage(message.content)}
                          </div>}
                        
                        <div className={cn("text-xs mt-3 opacity-70", message.role === 'user' ? "text-white/70" : "text-gray-500 dark:text-gray-400")}>
                          {message.timestamp.toLocaleTimeString([], {
                      hour: '2-digit',
                      minute: '2-digit'
                    })}
        ...
        <div className="flex-shrink-0 bg-white/80 dark:bg-black/20 backdrop-blur-sm border-t border-white/20 dark:border-white/10 p-6">
          <div className="max-w-4xl mx-auto">
            <PureMultimodalInput chatId={`arlo-desktop-${activeConversationId}`} messages={activeConversation?.messages || []} attachments={attachments} setAttachments={setAttachments} onSendMessage={handleSendMessage} onStopGenerating={handleStopGenerating} isGenerating={isLoading} canSend={true} selectedVisibilityType="private" value={inputValue} onChange={setInputValue} />
          </div>
        </div>
      </div>
    </div>;
