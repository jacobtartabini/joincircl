    ...
    return <div className="flex flex-col h-full bg-gray-50 p-4">
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto">
            {tabs.map(tab => <TabsContent key={tab.id} value={tab.id} className="mt-6 p-0">
                <tab.component />
              </TabsContent>)}
          </div>
        </Tabs>
                ...
                <Card className="h-full border-0 shadow-sm bg-white">
                  <CardContent className="p-0">
                    <TabsList className="flex flex-col h-auto w-full bg-transparent p-2 space-y-1">
                      {tabs.map(tab => <TabsTrigger key={tab.id} value={tab.id} className="w-full justify-start gap-3 px-4 py-3 text-left data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 hover:bg-gray-50 transition-colors rounded-full">
                          <tab.icon className="h-4 w-4" />
                          <span className="font-medium">{tab.label}</span>
                        </TabsTrigger>)}
                    </TabsList>
