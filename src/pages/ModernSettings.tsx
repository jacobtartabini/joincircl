...
const ModernSettings = () => {
              ...
              <div className="flex overflow-x-auto scrollbar-hide px-4 py-2 gap-1">
                {tabs.map((tab) => (
                  <TabsTrigger
                    key={tab.id}
                    value={tab.id}
                    className="flex-shrink-0 flex items-center gap-2 px-4 py-2.5 text-sm font-medium rounded-lg whitespace-nowrap data-[state=active]:bg-primary/10 data-[state=active]:text-primary data-[state=active]:border-primary/20 hover:bg-accent transition-all duration-200 border border-transparent text-muted-foreground"
                  >
          ...
          <div className="flex-1 overflow-y-auto min-h-0">
            {tabs.map((tab) => (
              <TabsContent 
                key={tab.id} 
                value={tab.id} 
                className="mt-0 p-4 h-full data-[state=active]:flex data-[state=active]:flex-col"
              >
  ...
  return (
    <div className="h-full unified-web-theme">
                  ...
                  <TabsList className="flex flex-col h-auto w-full bg-transparent p-2 space-y-1">
                    {tabs.map((tab) => (
                      <TabsTrigger
                        key={tab.id}
                        value={tab.id}
                        className="w-full justify-start gap-3 px-4 py-3 text-left rounded-lg data-[state=active]:bg-primary/10 data-[state=active]:text-primary hover:bg-accent transition-colors text-muted-foreground"
                      >
                        <tab.icon className="h-4 w-4" />
