...
export default function MobileKeystones() {
  ...
  const KeystoneCard = ({ keystone, isPast = false }: { keystone: Keystone; isPast?: boolean }) => {
          ...
          {categorizedKeystones.upcoming.length > 0 ? (
            <AnimatePresence mode="popLayout">
              {categorizedKeystones.upcoming.map(keystone => (
                <KeystoneCard key={keystone.id} keystone={keystone} />
              ))}
            </AnimatePresence>
          ) : (
          ...
          <section>
            ...
            <AnimatePresence mode="popLayout">
              {categorizedKeystones.past.map(keystone => (
                <KeystoneCard key={keystone.id} keystone={keystone} isPast />
              ))}
            </AnimatePresence>
          </section>
        )}
...
function KeystoneFormContent({ 
  ...
  return (
    <MobileForm
      ...
      onCancel={onCancel}
      submitLabel={keystone ? "Update" : "Create"}
    >
      <MobileInput
        label="Title *"
