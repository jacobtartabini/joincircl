      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {prompts.map((prompt, index) => (
          <Button
            ...
            className="h-auto p-4 text-left justify-start gap-3 glass-card hover:glass-card-enhanced transition-all rounded-2xl"
          >
            <div className="p-2 rounded-lg bg-primary/10 flex-shrink-0">
              <prompt.icon className="h-5 w-5 text-primary" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="font-medium text-foreground break-words pr-2">
                {prompt.text}
              </div>
