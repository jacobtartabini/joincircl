      ...
      <div className="space-y-6">
        <Card className="p-6">
          <div className="space-y-4">
            ...
            <MultiContactSelector contacts={contacts} selectedContacts={selectedContacts} onSelectionChange={setSelectedContacts} label="Select relevant contacts" placeholder="Search by name, company, or job title..." />

            {selectedContacts.length > 0 && <div className="mt-4 p-4 bg-blue-50 rounded-lg">
                <h6 className="text-sm font-medium text-blue-900 mb-2">
                  Selected Network Contacts ({selectedContacts.length})
                </h6>
                <div className="space-y-2">
                  {selectedContacts.map(contact => <div key={contact.id} className="text-sm text-blue-800">
                      <span className="font-medium">{contact.name}</span>
                      {contact.job_title && contact.company_name && <span className="text-blue-600"> - {contact.job_title} at {contact.company_name}</span>}
                    </div>)}
                </div>
              </div>}
          </div>
