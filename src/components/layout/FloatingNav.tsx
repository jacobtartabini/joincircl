...
export default function FloatingNav() {
  ...
  return <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-50">
      <div className="flex items-center gap-2 border bg-white/95 backdrop-blur-sm p-1 shadow-lg rounded-full">
        {tabs.map((tab, index) => {
        if ('type' in tab && tab.type === "separator") {
          return <div key={`separator-${index}`} className="mx-1 h-[24px] w-[1.2px] bg-border" aria-hidden="true" />;
        }
        ...
        return <Link key={navTab.title} to={navTab.path} className={`relative flex items-center rounded-xl px-4 py-2 text-sm font-medium transition-all duration-300 ${isSelected ? "bg-muted text-circl-blue gap-2" : "text-muted-foreground hover:bg-muted hover:text-foreground gap-0"}`} onClick={() => handleTabChange(index)}>
              <Icon size={20} />
