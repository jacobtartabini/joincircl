...
export function UnifiedContactView({
  ...
  const ContactInfoItem = ({
    ...
    const content = <div className="unified-button flex items-center gap-3 py-2 px-3 transition-all duration-200 hover:bg-white/30 dark:hover:bg-white/5 rounded-lg">
  ...
  const timelineItems = [...interactions.map(item => ({
  ...
  return <>
              ...
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-6">
                  <Avatar className="h-16 w-16 border-2 border-white/20 dark:border-white/10">
                    {contact.avatar_url ? <AvatarImage src={contact.avatar_url} alt={contact.name} /> : <AvatarFallback className="text-xl font-medium bg-gradient-to-br from-[#0daeec] to-[#0daeec]/70 text-white">
                  ...
                  <div className="space-y-2">
                    ...
                    {contact.job_title && contact.company_name && <p className="text-base text-muted-foreground font-medium">
                        {contact.job_title} at {contact.company_name}
                      </p>}
                    {contact.location && <p className="text-sm text-muted-foreground flex items-center gap-2">
              ...
              <Tabs defaultValue="about" className="w-full">
                ...
                <TabsContent value="about" className="space-y-6">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="unified-container p-5 rounded-lg">
                        ...
                        <div className="space-y-3">
                          ...
                          {contact.website && <ContactInfoItem icon={Globe} label="Website" value={contact.website} href={contact.website} />}
                        </div>
                      </div>

                      {/* Professional Information */}
                      {(contact.company_name || contact.industry || contact.department || contact.work_address) && <div className="unified-container p-5 rounded-lg">
                          <h2 className="text-lg font-semibold text-foreground mb-4">Professional</h2>
                          <div className="space-y-3">
                            {contact.company_name && <ContactInfoItem icon={Building} label="Company" value={contact.company_name} />}
                            {contact.job_title && <ContactInfoItem icon={Briefcase} label="Job Title" value={contact.job_title} />}
                            {contact.industry && <ContactInfoItem icon={Briefcase} label="Industry" value={contact.industry} />}
                            {contact.department && <ContactInfoItem icon={Briefcase} label="Department" value={contact.department} />}
                            {contact.work_address && <ContactInfoItem icon={MapPin} label="Office" value={contact.work_address} />}
                          </div>
                        </div>}

                      {/* Social Media */}
                      {(contact.linkedin || contact.twitter || contact.facebook || contact.instagram) && <div className="unified-container p-5 rounded-lg">
                          <h2 className="text-lg font-semibold text-foreground mb-4">Social Media</h2>
                          <div className="space-y-3">
                            {contact.linkedin && <ContactInfoItem icon={Linkedin} label="LinkedIn" value={contact.linkedin} href={contact.linkedin} />}
                            {contact.twitter && <ContactInfoItem icon={MessageSquare} label="Twitter" value={`@${contact.twitter}`} href={`https://twitter.com/${contact.twitter}`} />}
                            {contact.facebook && <ContactInfoItem icon={Facebook} label="Facebook" value={contact.facebook} href={contact.facebook} />}
                            {contact.instagram && <ContactInfoItem icon={Instagram} label="Instagram" value={`@${contact.instagram}`} href={`https://instagram.com/${contact.instagram}`} />}
                          </div>
                        </div>}
                    </div>
                    ...
                    <div className="space-y-6">
                      {/* Education */}
                      {(contact.university || contact.major || contact.minor || contact.graduation_year) && <div className="unified-container p-5 rounded-lg">
                          <h2 className="text-lg font-semibold text-foreground mb-4">Education</h2>
                          <div className="space-y-3">
                            {contact.university && <ContactInfoItem icon={GraduationCap} label="University" value={contact.university} />}
                            {contact.major && <ContactInfoItem icon={GraduationCap} label="Major" value={contact.major} />}
                            {contact.minor && <ContactInfoItem icon={GraduationCap} label="Minor" value={contact.minor} />}
                            {contact.graduation_year && <ContactInfoItem icon={Calendar} label="Graduated" value={contact.graduation_year.toString()} />}
                          </div>
                        </div>}

                      {/* Personal Information */}
                      {(contact.birthday || contact.how_met || contact.hobbies_interests) && <div className="unified-container p-5 rounded-lg">
                          <h2 className="text-lg font-semibold text-foreground mb-4">Personal</h2>
                          <div className="space-y-3">
                            {contact.birthday && <ContactInfoItem icon={Gift} label="Birthday" value={format(new Date(contact.birthday), 'MMM dd, yyyy')} />}
                            {contact.how_met && <div className="unified-container p-3 rounded-lg">
                            ...
                            {contact.hobbies_interests && <div className="unified-container p-3 rounded-lg">
                      ...
                      {/* Tags */}
                      {contact.tags && contact.tags.length > 0 && <div className="unified-container p-5 rounded-lg">
                          <h2 className="text-lg font-semibold text-foreground mb-4">Tags</h2>
                          <div className="flex flex-wrap gap-2">
                            {contact.tags.map(tag => <span key={tag} className="px-2 py-1 text-xs font-medium bg-[#0daeec]/10 text-[#0daeec] border border-[#0daeec]/20 rounded-full">
                                {tag}
                              </span>)}
                          </div>
                        </div>}
                      ...
                      {/* Notes */}
                      {contact.notes && <div className="unified-container p-5 rounded-lg">
                ...
                <TabsContent value="timeline">
                  ...
                  {timelineItems.length === 0 ? <div className="text-center py-12 text-muted-foreground">
                      ...
                      {timelineItems.map(item => <div key={item.id} className="unified-container p-4 rounded-lg border-l-4 border-[#0daeec]/30">
      ...
      <Dialog open={isAddInteractionOpen} onOpenChange={setIsAddInteractionOpen}>
        <DialogContent className="glass-card-enhanced border-white/20 dark:border-white/15">
          ...
          <InteractionForm contact={contact} onSuccess={() => {
      ...
      <Dialog open={isAddKeystoneOpen} onOpenChange={setIsAddKeystoneOpen}>
        <DialogContent className="glass-card-enhanced border-white/20 dark:border-white/15">
          ...
          <KeystoneForm contact={contact} onSuccess={() => {
