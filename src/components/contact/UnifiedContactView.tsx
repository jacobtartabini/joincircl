...
export function UnifiedContactView({
  ...
  const ContactInfoItem = ({
    ...
    const content = <div className="flex items-center gap-3 py-2 hover:bg-gray-50/50 rounded-lg transition-colors px-3 -mx-3">
  ...
  return <>
        ...
        <div className="px-8 py-12">
          ...
          <div className="flex items-start justify-between mb-16">
            <div className="flex items-center gap-8">
              <Avatar className="h-20 w-20">
                {contact.avatar_url ? <AvatarImage src={contact.avatar_url} alt={contact.name} /> : <AvatarFallback className="text-2xl font-light bg-gradient-to-br from-blue-500 to-purple-600 text-white">
              ...
              <div className="space-y-2">
                ...
                {contact.location && <p className="text-sm text-gray-500 flex items-center gap-1">
            ...
            <div className="flex items-center gap-3">
              <Button variant="outline" size="sm" onClick={() => setIsAddInteractionOpen(true)} className="text-gray-600 hover:text-gray-900 rounded-full">
                <Plus className="h-4 w-4 mr-2" />
                Add Interaction
              </Button>
              <Button variant="outline" size="sm" onClick={() => setIsAddKeystoneOpen(true)} className="text-gray-600 hover:text-gray-900 rounded-full">
                <Plus className="h-4 w-4 mr-2" />
                Add Keystone
              </Button>
              <Button variant="outline" size="sm" onClick={onEdit} className="text-gray-600 hover:text-gray-900 rounded-full">
                <Edit className="h-4 w-4 mr-2" />
                Edit
              </Button>
              <Button variant="outline" size="sm" onClick={onDelete} className="text-red-600 hover:text-red-700 border-red-200 hover:bg-red-50 rounded-full">
          ...
          <div className="grid grid-cols-12 gap-16">
            ...
            <div className="col-span-4 space-y-12">
              ...
              {(contact.company_name || contact.industry || contact.department || contact.work_address) && <div className="space-y-4">
              ...
              {/* Education */}
              {(contact.university || contact.major || contact.graduation_year) && <div className="space-y-4">
                  ...
                  <div className="space-y-1">
                    ...
                    {contact.major && <ContactInfoItem icon={GraduationCap} label="Major" value={`${contact.major}${contact.minor ? ` • Minor: ${contact.minor}` : ''}`} />}
                    {contact.graduation_year && <ContactInfoItem icon={Calendar} label="Graduated" value={contact.graduation_year.toString()} />}
                  </div>
                </div>}

              {/* Personal Information */}
              {(contact.birthday || contact.how_met || contact.hobbies_interests) && <div className="space-y-4">
                  <h2 className="text-sm font-semibold text-gray-900 tracking-wide uppercase">Personal</h2>
                  <div className="space-y-3">
                    {contact.birthday && <ContactInfoItem icon={Gift} label="Birthday" value={new Date(contact.birthday).toLocaleDateString()} />}
                    {contact.how_met && <div className="py-2">
                    ...
                    {contact.hobbies_interests && <div className="py-2">
            ...
            <div className="col-span-8">
              <div className="space-y-8">
                ...
                {timelineItems.length === 0 ? <div className="text-center py-16 text-gray-500">
                    ...
                    {timelineItems.map(item => <div key={item.id} className="flex gap-4 pb-6 border-b border-gray-100 last:border-b-0">
      ...
      <Dialog open={isAddInteractionOpen} onOpenChange={setIsAddInteractionOpen}>
        <DialogContent className="max-w-md">
          ...
          <InteractionForm contact={contact} onSuccess={async () => {
      ...
      <Dialog open={isAddKeystoneOpen} onOpenChange={setIsAddKeystoneOpen}>
        <DialogContent className="max-w-md">
          ...
          <KeystoneForm contact={contact} onSuccess={async () => {
