...
export function EnhancedContactDetail({
  ...
  const ContactInfoSection = () => <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        {contact.personal_email && <Button variant="outline" size="sm" onClick={() => window.open(`mailto:${contact.personal_email}`)} className="unified-button justify-start gap-2 h-9 rounded-full">
            <Mail className="h-4 w-4" />
            Email
          </Button>}
        {contact.mobile_phone && <Button variant="outline" size="sm" onClick={() => window.open(`tel:${contact.mobile_phone}`)} className="unified-button justify-start gap-2 h-9 rounded-full">
  ...
  const ContactDetailsSection = () => <div className="space-y-4">
      ...
      {contact.tags && contact.tags.length > 0 && <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-1">
            {contact.tags.slice(0, 6).map(tag => <Badge key={tag} variant="outline" className="text-xs bg-gradient-to-r from-[#0daeec]/10 to-[#0daeec]/5 text-[#0daeec] border border-[#0daeec]/20">
                {tag}
              </Badge>)}
  ...
  const RecentActivitySection = () => <div className="space-y-4">
      ...
      {interactions.length === 0 ? <div className="text-center py-6 text-muted-foreground">
          ...
          {interactions.slice(0, 3).map(interaction => <div key={interaction.id} className="unified-container flex gap-3 p-3 rounded-lg">
          ...
          {interactions.length > 3 && <Button variant="ghost" size="sm" onClick={onViewAll} className="unified-button w-full text-sm text-muted-foreground hover:text-foreground">
  ...
  // Desktop panel
  return <div className="h-full flex flex-col glass-card-enhanced">
      {/* Header */}
