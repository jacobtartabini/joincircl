...
import React, { useRef, useEffect, useState, useCallback, type Dispatch, type SetStateAction, type ChangeEvent, memo } from 'react';
...
const StopIcon = ({
  ...
  return <svg height={size} viewBox="0 0 16 16" width={size} style={{
    color: 'currentcolor'
  }}>
...
const PaperclipIcon = ({
  ...
  return <svg height={size} strokeLinejoin="round" viewBox="0 0 16 16" width={size} style={{
    color: 'currentcolor'
  }} className="-rotate-45">
...
const PreviewAttachment = ({
  ...
  return <div data-testid="input-attachment-preview" className="flex flex-col gap-1">
      ...
      <img key={url} src={url} alt={name ?? 'An image attachment'} className="rounded-md size-full object-cover grayscale" /> : <div className="flex items-center justify-center text-xs text-gray-600 text-center p-1">
             File: {name?.split('.').pop()?.toUpperCase() || 'Unknown'}
          </div>}

        {isUploading && <div data-testid="input-attachment-loader" className="animate-spin absolute text-gray-500">
...
function PureAttachmentsButton({
  ...
  return <Button data-testid="attachments-button" className="rounded-md rounded-bl-lg p-[7px] h-fit border border-gray-300 hover:bg-gray-200" onClick={event => {
    event.preventDefault();
    fileInputRef.current?.click();
  }} disabled={disabled} variant="ghost" // ghost variant now has text-black
...
function PureSendButton({
  ...
  className="rounded-full p-1.5 h-fit" onClick={event => {
    event.preventDefault();
    if (!isDisabled) {
      submitForm();
    }
  }} disabled={isDisabled} aria-label="Send message">
      <ArrowUpIcon size={14} />
    </Button>;
}
...
function PureMultimodalInput({
  ...
  const handleFileChange = useCallback(async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const successfullyUploadedAttachments = uploadedAttachments.filter((attachment): attachment is Attachment => attachment !== undefined);
  ...
  const handleRemoveAttachment = useCallback((attachmentToRemove: Attachment) => {
    ...
    setAttachments(currentAttachments => currentAttachments.filter(attachment => attachment.url !== attachmentToRemove.url || attachment.name !== attachmentToRemove.name));
    // Focus the textarea
  ...
  const submitForm = useCallback(() => {
    ...
    resetHeight();
    textareaRef.current?.focus();
  }, [input, attachments, onSendMessage, setAttachments, setInput, textareaRef, resetHeight]);
  ...
  return <div className={cn("relative w-full flex flex-col gap-4", className)}>

      <AnimatePresence>
       {showSuggestedActions && <motion.div key="suggested-actions-container" initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
        ...
        duration: 0.2
            ...
            <SuggestedActions onSelectAction={action => {
          setInput(action);
          requestAnimationFrame(() => {
            adjustHeight();
            textareaRef.current?.focus();
          });
        }} chatId={chatId} selectedVisibilityType={selectedVisibilityType} />
         </motion.div>}
      </AnimatePresence>


      {/* Hidden file input */}
      <input type="file" className="fixed -top-4 -left-4 size-0.5 opacity-0 pointer-events-none" ref={fileInputRef} multiple onChange={handleFileChange} tabIndex={-1} disabled={isAttachmentDisabled} accept="image/*,video/*,audio/*,.pdf" // Example mime types
    />

      {(attachments.length > 0 || uploadQueue.length > 0) && <div data-testid="attachments-preview" className="flex pt-[10px] flex-row gap-3 overflow-x-auto items-end pb-2 pl-1">
          {attachments.map(attachment => <div key={attachment.url || attachment.name} className="relative group">
          ...
          {uploadQueue.map((filename, index) => <PreviewAttachment key={`upload-${filename}-${index}`} attachment={{
        url: '',
        name: filename,
        contentType: '',
        size: 0
      ...
      <Textarea data-testid="multimodal-input" ref={textareaRef} placeholder="Ask Arlo about your network..." value={input} onChange={handleInput} className={cn('min-h-[24px] max-h-[calc(75dvh)] overflow-y-auto resize-none rounded-2xl !text-base pb-10', 'bg-gray-100 border border-gray-300', className)} style={{
      color: 'black'
    }} rows={1} autoFocus disabled={!canSend || isGenerating || uploadQueue.length > 0} onKeyDown={event => {
      ...
      <div className="absolute bottom-0 right-0 p-2 w-fit flex flex-row justify-end z-10">
        {isGenerating ? <StopButton onStop={onStopGenerating} /> : <SendButton submitForm={submitForm} input={input} uploadQueue={uploadQueue} attachments={attachments} canSend={canSend} isGenerating={isGenerating} />}
      </div>
    </div>;
}
