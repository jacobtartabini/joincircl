...
import { add, eachDayOfInterval, endOfMonth, endOfWeek, format, getDay, isEqual, isSameDay, isSameMonth, isToday, parse, startOfToday, startOfWeek, startOfYear, endOfYear, eachMonthOfInterval, getDaysInMonth, startOfMonth } from "date-fns";
...
export function FullCalendar({
          ...
          <div className="p-4 space-y-1">
            ...
            >
                  ...
                  <div className="flex-1 p-2 space-y-1">
                    {hourEvents.map(event => <motion.div key={event.id} onClick={() => onEventClick?.(event)} className={cn("p-2 rounded-md border cursor-pointer text-sm", eventTypeColors[event.type])} whileHover={{
                  scale: 1.02
                }}>
        ...
        <div className="min-h-full bg-white dark:bg-gray-900">
          <div className="sticky top-0 z-10 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
            <div className="grid grid-cols-8 border-b border-gray-200 dark:border-gray-700">
              ...
              {weekDays.map(day => <div key={day.toString()} className="p-4 text-center border-l border-gray-200 dark:border-gray-700">
                  <div className="text-sm font-medium">{format(day, 'EEE')}</div>
                  <div className={cn("text-lg font-semibold mt-1", isToday(day) && "text-blue-600 dark:text-blue-400")}>
                    {format(day, 'd')}
                  </div>
                </div>)}
            </div>
          ...
          <div className="space-y-0">
            {hours.map(hour => <div key={hour} className="grid grid-cols-8 border-b border-gray-100 dark:border-gray-800 min-h-16">
  ...
  const renderMonthView = () => <div className="flex-1 overflow-auto">
      <div className="bg-white dark:bg-gray-900 h-full">
        ...
        <div className="hidden lg:block h-full">
          <div className="grid grid-cols-7 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
            {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day, index) => <div key={day} className="p-3 text-center text-sm font-semibold border-r border-gray-200 dark:border-gray-700 last:border-r-0">
                {day}
              </div>)}
          </div>
            ...
            {days.map((day, dayIdx) => {
              ...
              >
                ...
                <div className="flex-1 p-2 space-y-1 overflow-hidden">
                  {data.filter(event => isSameDay(event.day, day)).map(dayData => <div key={dayData.day.toString()}>
                        {dayData.events.slice(0, 3).map(event => <Popover key={event.id}>
                            <PopoverTrigger asChild>
                              <motion.div onClick={e => {
                      e.stopPropagation();
                    }} className={cn("p-1 rounded text-xs cursor-pointer border", eventTypeColors[event.type])} whileHover={{
                      scale: 1.02
                    }}>
                                <div className="font-medium truncate">{event.name}</div>
                                <div className="opacity-75">{event.time}</div>
                            ...
                            <PopoverContent className="w-80 p-3">
                              <div className="space-y-2">
                                ...
                                <Button size="sm" onClick={() => onEventClick?.(event)} className="w-full">
                                  View Details
                                </Button>
                              </div>
                            </PopoverContent>
                          </Popover>)}
                        {dayData.events.length > 3 && <div className="text-xs text-gray-500 px-1">
        ...
        <div className="lg:hidden">
          <div className="grid grid-cols-7 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
            {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, index) => <div key={day} className="p-2 text-center text-sm font-semibold">
                {day}
              </div>)}
          </div>
          <div className="grid grid-cols-7 gap-px bg-gray-200 dark:bg-gray-700">
            {days.map((day, dayIdx) => <button key={dayIdx} onClick={() => setSelectedDay(day)} onDoubleClick={() => handleDoubleClick(day)} className={cn("bg-white dark:bg-gray-900 p-2 h-16 text-sm relative", !isSameMonth(day, firstDayCurrentMonth) && "text-gray-400 bg-gray-50 dark:bg-gray-800", isEqual(day, selectedDay) && "bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400", isToday(day) && "bg-blue-600 text-white")}>
                <div>{format(day, 'd')}</div>
                {data.filter(date => isSameDay(date.day, day)).length > 0 && <div className="absolute bottom-1 left-1/2 transform -translate-x-1/2 flex space-x-1">
                    {data.filter(date => isSameDay(date.day, day)).slice(0, 1).map(date => <div key={date.day.toString()} className="flex space-x-1">
                          {date.events.slice(0, 3).map(event => <div key={event.id} className={cn("w-1.5 h-1.5 rounded-full", event.type === 'keystone' && "bg-blue-500", event.type === 'interaction' && "bg-green-500", event.type === 'birthday' && "bg-pink-500", event.type === 'sync' && "bg-purple-500", event.type === 'calendar' && "bg-orange-500")} />)}
                        </div>)}
                  </div>}
              </button>)}
          </div>
  ...
  const renderYearView = () => {
    ...
    return <div className="flex-1 overflow-auto p-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 auto-rows-max">
          ...
          return <div key={month.toString()} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                <h3 className="text-sm font-semibold text-center mb-3">
                  {format(month, 'MMMM yyyy')}
                </h3>
                <div className="grid grid-cols-7 gap-1 text-xs">
                  {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map(day => <div key={day} className="text-center text-gray-500 font-medium p-1">
                      {day}
                    </div>)}
                  {monthDays.map((day, dayIdx) => {
                const dayEvents = data.find(d => isSameDay(d.day, day))?.events || [];
                return <button key={dayIdx} onClick={() => setSelectedDay(day)} onDoubleClick={() => handleDoubleClick(day)} className={cn("p-1 text-xs rounded hover:bg-gray-100 dark:hover:bg-gray-700 relative", !isSameMonth(day, month) && "text-gray-300 dark:text-gray-600", isToday(day) && "bg-blue-600 text-white", isEqual(day, selectedDay) && !isToday(day) && "bg-blue-100 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400")}>
                        {format(day, 'd')}
                        {dayEvents.length > 0 && <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 flex space-x-0.5">
                            {dayEvents.slice(0, 3).map((event, idx) => <div key={idx} className={cn("w-1 h-1 rounded-full", event.type === 'keystone' && "bg-blue-500", event.type === 'interaction' && "bg-green-500", event.type === 'birthday' && "bg-pink-500", event.type === 'sync' && "bg-purple-500", event.type === 'calendar' && "bg-orange-500")} />)}
                          </div>}
                      </button>;
              })}
                </div>
  ...
  return <div className="flex flex-col h-full min-h-[600px] bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700">
      {/* Header */}
      <div className="flex flex-col space-y-4 p-6 md:flex-row md:items-center md:justify-between md:space-y-0 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
        <div className="flex flex-col md:flex-row md:items-center gap-4">
          <div className="flex items-center gap-4">
            <div className="flex flex-col">
              ...
              {view === 'day' && <p className="text-sm text-gray-600 dark:text-gray-400">
                  {format(selectedDay, 'EEEE, MMMM d, yyyy')}
                </p>}
              {view === 'week' && <p className="text-sm text-gray-600 dark:text-gray-400">
          ...
          <div className="flex rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
            {['day', 'week', 'month', 'year'].map(v => <Button key={v} onClick={() => setView(v as any)} variant={view === v ? 'default' : 'ghost'} size="sm" className="rounded-none capitalize">
                {v}
              </Button>)}
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="flex rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
            <Button onClick={view === 'year' ? () => setCurrentMonth(format(add(firstDayCurrentMonth, {
            years: -1
          }), "MMM-yyyy")) : previousMonth} variant="ghost" size="sm" className="rounded-none">
              <ChevronLeftIcon className="h-4 w-4" />
            </Button>
            <Button onClick={goToToday} variant="ghost" size="sm" className="rounded-none px-4">
              Today
            </Button>
            <Button onClick={view === 'year' ? () => setCurrentMonth(format(add(firstDayCurrentMonth, {
            years: 1
          }), "MMM-yyyy")) : nextMonth} variant="ghost" size="sm" className="rounded-none">
