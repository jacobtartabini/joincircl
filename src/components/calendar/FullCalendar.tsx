...
export function FullCalendar({ onCreateEvent }: FullCalendarProps) {
        ...
        <div className="grid grid-rows-24 gap-0 min-h-full">
          {hours.map(hour => {
            ...
            return (
              <div
                key={hour}
                className="border-b border-gray-200 dark:border-gray-700 p-2 min-h-[60px] hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer"
                onDoubleClick={() => handleDoubleClick(currentDate, `${hour.toString().padStart(2, '0')}:00`)}
              >
                ...
                {timeEvents.map(event => (
                  <Popover key={event.id}>
                    <PopoverTrigger asChild>
                      <div
                        className={`${getEventTypeColor(event.type)} text-white text-xs p-1 rounded mb-1 cursor-pointer hover:opacity-80`}
            ...
            {weekDays.map(day => {
              ...
              return (
                <div
                  ...
                  className="border-r border-gray-200 dark:border-gray-700 p-1 min-h-[50px] hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer"
              ...
              <div className="space-y-1">
                {dayEvents.slice(0, 3).map(event => (
                  <Popover key={event.id}>
                    <PopoverTrigger asChild>
                      <div
                        className={`${getEventTypeColor(event.type)} text-white text-xs p-1 rounded cursor-pointer hover:opacity-80 truncate`}
                        onClick={() => handleEventClick(event)}
                      >
                        {event.title}
                      </div>
              ...
              <CardContent>
                ...
                <div className="space-y-1">
                  {monthEvents.slice(0, 3).map(event => (
                    <div key={event.id} className="text-xs p-1 bg-gray-100 dark:bg-gray-800 rounded truncate">
                      {event.title}
                    </div>
                  ))}
